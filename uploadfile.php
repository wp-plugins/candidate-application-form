...
if(isset($_REQUEST["delteOldFile"]) && $_REQUEST["delteOldFile"]!='' && isset($_REQUEST["name"]) && $_REQUEST["name"]=='nofile'){
	//$fileToBeDelete = CRINCH_PDF_UPLOAD_DIR.$_REQUEST["delteOldFile"];
	$fileToBeDelete = $targetDir.DIRECTORY_SEPARATOR .$_REQUEST["delteOldFile"];
	@unlink($fileToBeDelete);
	die();
}

if(isset($_REQUEST["delteOldFile"]) && $_REQUEST["delteOldFile"]!='' && $_REQUEST["name"]!='nofile'){
	//$fileToBeDelete = CRINCH_PDF_UPLOAD_DIR.$_REQUEST["delteOldFile"];  
	$fileToBeDelete = $targetDir.DIRECTORY_SEPARATOR.$_REQUEST["delteOldFile"];
	if(file_exists($fileToBeDelete)){
		@unlink($fileToBeDelete);
	}
	/*if (!unlink($fileToBeDelete))
	{
		echo ("Error deleting $fileToBeDelete");
		die();
	}*/
} 
...
$uploaded_filepath = $exploded_filepath[1];
